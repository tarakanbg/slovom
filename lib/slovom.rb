...
module Slovom
  class Verbalizer
    ...
    def initialize(decimal)
      ...
      @stotinki_title = @stotinki == 1 ? ' стотинка' : ' стотинки'
      @too_big = 'много'
      @output = ''
  ...
  private
    ...
    def numbers_slovom(amount, feminine = nil)
      case amount
      when 1..99 then below_hundred(amount, feminine)
      when 100..999 then hundreds(amount, feminine)
      when 1000..999999 then thousands(amount, feminine)
      when 1000000..999999999999999 then gazillions(amount, feminine)
      else
        @too_big
      end
    end
    ...
    def gazillions(digits, feminine = nil)
      ...
      word = big_number == 1 ? word_singular : word_plural
